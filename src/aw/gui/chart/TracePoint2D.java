 ...
 * aggregators (<code>ITrace2D</code>) complete tracepoints. This avoids the
 * necessarity to care for the correct order of a set of scaled tracepoints
 * copied for caching purposes. Especially in the case of new
...
public class TracePoint2D extends Point2D.Double implements Comparable, java.io.Serializable,
   ...
   * <p>
   */
  private AbstractTrace2D m_listener;
   ...
   * Modifications of this instances by
   * <code>setLocations(double x,double y)</code> will cause of invocation of
  ...
  public TracePoint2D(final ITrace2D listener) {
    super();
    this.m_listener = (AbstractTrace2D) listener;
  }
   ...
   *          deregister.
   */
  void setListener(final AbstractTrace2D listener) {
