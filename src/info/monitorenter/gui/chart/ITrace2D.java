...
/**
 * An interface used by <code>Chart2D</code>. ITrace2D contains the values to display, the color
 * for displaying and a suitable label. It may be seen as a trace of the <code>Chart2D</code> that
 * displays it. <br>
 * Implementations may be optimized for different use- cases: <br>
 * RingBuffers for fast changing data to keep the amount of tracepoints and consumption of memory
 * constant, internal Lists for allowing the sorting the internal <code>TracePoint2D</code>-
 * instances or Arrays for unordered Data (in the order of adding) for fast performance. Even an
 * <code>ITrace2D</code> constructed by a "function- Object" may be thinkable.
 * <p>
 * There are various constraints for Traces: <br>- ordered by x-values <br>- ordered by the order
 * of <code>addPoint</code>- invocation (control form outside) <br>- unique, single x- values
 * <br>- limitation of tracepoints <br>- time- critical (fast- changing tracepoints) <br>
 * <br>
 * Therefore there are various <code>ITrace2D</code>- implementations. Read their description to
 * find the one you need. Some may not have been written yet.
 * <p>
 * {@link java.lang.Comparable} should be implemented by using the internal property zIndex (see
 * {@link #getZIndex()}, {@link #setZIndex(Integer)}).
 * <p>
 * <h3>Property Change events</h3>
 * The following <code>{@link java.beans.PropertyChangeEvent}</code> may be fired to
 * <code>{@link PropertyChangeListener}</code> instances that register themselves with
 * <code>{@link #addPropertyChangeListener(String, PropertyChangeListener)}</code>. <table
 * border="0">
 * <tr>
 ...
 * <td><code>{@link ITrace2D}</code> that changed</td>
 * <td><code>{@link info.monitorenter.gui.chart.TracePoint2D}</code>, the instance that was
 * removed</td>
 ...
 * <td><code>{@link info.monitorenter.gui.chart.TracePoint2D}</code>, the new instance that was
 * added, identifying that an instance was removed</td>
 * </tr>
 * <tr>
 ...
 * <td><code>{@link info.monitorenter.gui.chart.ITracePainter}</code>, the new painter.</td>
 * </tr>
 * <tr>
 * <td><code>{@link info.monitorenter.gui.chart.ITrace2D#PROPERTY_PAINTERS}</code></td>
 * <td><code>{@link ITrace2D}</code> that changed</td>
 * <td><code>{@link info.monitorenter.gui.chart.ITracePainter}</code>, the old painter.</td>
 ...
 * <td><code>{@link info.monitorenter.gui.chart.IErrorBarPolicy}</code>, the new error bar
 * policy.</td>
 * </tr>
 * <tr>
 * <td><code>{@link info.monitorenter.gui.chart.ITrace2D#PROPERTY_ERRORBARPOLICY}</code></td>
 * <td><code>{@link ITrace2D}</code> that changed</td>
 * <td><code>{@link info.monitorenter.gui.chart.IErrorBarPolicy}</code>, the old error bar
 * policy.</td>
 ...
 * <td><code>{@link ITrace2D}</code> that notifies the change of the configured error bar policy.</td>
 * <td>null</td>
 * <td><code>{@link info.monitorenter.gui.chart.IErrorBarPolicy}</code>, the instance with the
 * configuration change.</td>
 * </tr>
 * <tr>
 * <td><code>{@link info.monitorenter.gui.chart.ITrace2D#PROPERTY_POINT_CHANGED}</code></td>
 * <td><code>{@link ITrace2D}</code> that notifies the change of the contained trace point.</td>
 * <td><code>null</code> because it would be too much overhead to store the old point in a
 * additional instance as the original is modified in
...
public interface ITrace2D extends IComparableProperty, PropertyChangeListener {
  ...
  /**
   * The property key defining the <code>color</code> property. Use in combination with
   * {@link #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
  public static final String PROPERTY_COLOR = "ITrace2D.PROPERTY_COLOR";

  /**
   * The property key defining a change of <code>{@link IErrorBarPolicy}</code> instances
   * contained.
   * <p>
   ...
   * <p>
   * Use in combination with {@link #addPropertyChangeListener(String, PropertyChangeListener)}.
   * <p>
   */
   ...
   * <p>
   * This is fired whenever an <code>IErrorBarPolicy</code> notifies this instance of a
   * configuration change via an event for
  ...
  /**
   * The property key defining the <code>maxX</code> property. Use in combination with
   * {@link #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
  public static final String PROPERTY_MAX_X = "ITrace2D.PROPERTY_MAX_X";

  /**
   * The property key defining the <code>maxY</code> property. Use in combination with
   * {@link #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
  public static final String PROPERTY_MAX_Y = "ITrace2D.PROPERTY_MAX_Y";

  /**
   * The property key defining the <code>minX</code> property. Use in combination with
   * {@link #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
  public static final String PROPERTY_MIN_X = "ITrace2D.PROPERTY_MIN_X";

  /**
   * The property key defining the <code>minY</code> property. Use in combination with
   * {@link #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
  public static final String PROPERTY_MIN_Y = "ITrace2D.PROPERTY_MIN_Y";

  /**
   * The property key defining the <code>name</code> property. Use in combination with
   * {@link #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
  public static final String PROPERTY_NAME = "ITrace2D.PROPERTY_NAME";

  /**
   * The property key defining a change in the set of <code>{@link ITracePainter}</code>
  ...
  /**
   * The property key defining the <code>physicalUnits</code> property. Use in combination with
   * {@link #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
   ...
   * <code>{@link TracePoint2D#setLocation(double, double)}</code> of any contained point is
   * invoked.
   * <p>
   */
  ...
  /**
   * The property key defining the <code>stroke</code> property. Use in combination with
   * {@link #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
  public static final String PROPERTY_STROKE = "ITrace2D.PROPERTY_STROKE";

  /**
   * The property key defining a change in the collection of <code>{@link TracePoint2D}</code>
  ...
  /**
   * The property key defining the <code>visible</code> property. Use in combination with
   * {@link #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
   ...
   * @return true if the painter was added (same instance was not contained before).
   */
   ...
   * <p>
   * Try to pass instances of <code>TracePoint2D</code> to this instance instead of invoking
   * <code>{@link #addPoint(double, double)}</code> to increase performace. Else the given point
   ...
   * internal properties <code>minX</code>, <code>maxX</code>,<code>maxY</code> and
   * <code>minY</code> and also care about firing property change events for those properties by
   * method
  ...
  /**
   * Registers a property change listener that will be informed about changes of the property
   * identified by the given <code>propertyName</code>.
   * <p>
   * 
   * @param propertyName
   ...
   *            <code>propertyName</code> changes
   */
   ...
   * @return true if the painter was added (class of instance not contained before).
   */
  ...
  /**
   * Method to trigger by <code>{@link TracePoint2D#setLocation(double, double)}</code>,
  ...
  /**
   * Because the color is data common to a trace of a <code>Chart2D</code> it is stored here.
   * <p>
   * On the other hand only the corresponding <code>Chart2D </code> may detect the same color
   * chosen for different <code>IChart2D</code> instances to be displayed. Therefore it is allowed
   * to return null. This is a message to the <code>Chart2D</code> to leave it the choice of the
   * color. Then the <code>Chart2D</code> will chose a color not owned by another
   * <code>ITrace2D</code> instance managed and assign it to the null- returning instance.
   * <p>
   * The <code>Chart2D </code> will often call this method. So try to cache the value in
   * implementation and only check on modifications of <code>TracePoint</code> instances or on
   * <code>add</code>- invocations for changes.
   * <p>
   * 
   * @return The chosen java.awt.Color or null if the decision for the color should be made by the
   *         corresponding <code>Chart2D</code>.
   */
  Color getColor();
   ...
   * <p>
   * 
   * @return the <code>Set&lt;{@link IErrorBarPolicy}&gt;</code> that will be used to render
   *         error bars for this trace.
   */
  ...
  /**
   * Callback method for the <code>Chart2D</code> that returns a <code>String</code> describing
   * the label of the <code>ITrace2D</code> that will be displayed below the drawing area of the
   * <code>Chart2D</code>.
   * <p>
   * This method should be implemented and finalized ASAP in the inheritance tree and rely on the
   * property <code>name</code> and <code>physicalUnits</code>.
   * <p>
   * 
  ...
  /**
   * <p>
   * Returns the maximum amount of {@link TracePoint2D} instances that may be added. For
   * implementations that limit the maximum amount this is a reasonable amount. Non-limiting
   * implementations should return {@link Integer#MAX_VALUE}. This allows to detect the
  ...
  /**
   * Returns the maximum value to be displayed on the x- axis of the <code>Chart2D</code>.
  ...
  /**
   * Returns the maximum value to be displayed on the y- axis of the Chart2D. Implementations should
   * be synchronized for multithreaded use. No exception is thrown. In case of empty data (no
   * tracepoints) 0 should be returned. (watch division with zero).
   * <p>
   * 
   ...
   * <p>
   * Implementations should be synchronized for multithreaded use. No exception is thrown. In case
   * of empty data (no tracepoints) 0 should be returned. (watch division with zero).
   * <p>
   * The <code>Chart2D </code> will often call this method. So try to cache the value in
   * implementation and only check on modifications of <code>TracePoint</code> instances or on
   * <code>add</code>- invocations for changes.
   ...
   * 
   * @param property
   *            one of the constants with the <code>PROPERTY_</code> prefix defined in this class
   *            or subclasses.
   ...
   * <p>
   * 
   * @return The amount of <code>{@link TracePoint2D}</code> instances currently contained.
   */
  public int getSize();
  ...
  /**
   * Returns the <code>Set&lt;{@link ITracePainter}&gt;</code> that will be used to paint this
   * trace.
   * <p>
   * 
   * @return the <code>Set&lt;{@link ITracePainter}&gt;</code> that will be used to paint this
   *         trace.
   */
   ...
   * <p>
   * 
   * @return the z-index that will define the order in which this instance will be painted.
   */
  public Integer getZIndex();

  /**
   * Returns false if internal <code>{@link TracePoint2D}</code> instances are contained or true
   * if not.
   * <p>
   * 
   * @return <tt>false</tt> if internal <code>{@link TracePoint2D}</code> instances are
   *         contained or <tt>true</tt> if not.
   */
  ...
  /**
   * Returns an <code>Iterator</code> over the internal <code>{@link TracePoint2D}</code>
   * instances.
   * <p>
   * Implementations should be synchronized. This method is meant to allow modifications of the
   * intenal <code>TracePoint2D</code> instances, so the original points should be returned.
   * <p>
   * There is no guarantee that changes made to the contained tracepoints will be reflected in the
   * display immediately. The order the iterator returns the <code>TracePoint2D</code> instances
   * decides how the <code>Chart2D</code> will paint the trace.
   * <p>
   * 
   * @return an <code>Iterator</code> over the internal <code>{@link TracePoint2D}</code>
   *         instances.
   */
  public Iterator iterator();

  /**
   * Removes all internal <code>TracePoint2D</code>.{@link #isEmpty()} will return true
   * afterwards.
   * <p>
   */
   ...
   * 
   * @param property
   *            one of the constants with teh <code>PROPERTY_</code> prefix defined in this class
   *            or subclasses.
   * @param listener
   ...
   * @return true if a trace painter of the class of the given argument was removed.
   */
   ...
   * @return the <code>Set&lt;{@link IErrorBarPolicy}&gt;</code> that was used before.
   */
  public Set setErrorBarPolicy(IErrorBarPolicy errorBarPolicy);

  /**
   * Assingns a specific name to the <code>ITrace2D</code> which will be displayed by the
   * <code>Chart2D</code>.
   * <p>
   * 
   * @param name
   ...
   *  ITrace2D</code> (e.g.
   * Volt, Ohm, lux, ...) which will be displayed by the <code>{@link Chart2D}</code>
   * <p>
   * 
   * @param xunit
  ...
  /**
   * This is a callback from {@link Chart2D#addTrace(ITrace2D)} and must not be invoked from
  ...
  /**
   * Allows to specify the rendering of the ITrace2D. This Stroke will be assigned to the
   * {@link java.awt.Graphics2D} by the rendering {@link Chart2D} when painting this instance.
   * <p>
   * 
   * @param stroke
  ...
  /**
   * Sets the internal z-index property. This decides the order in which different traces within the
   * same <code>{@link Chart2D}</code> are painted.
   * <p>
   * The lower the given value is the more this trace will be brought to front.
   * <p>
   * The value must not be lower than {@link #Z_INDEX_MIN}(0) and higher than {@link #ZINDEX_MAX}(100).
   * <p>
   * This might not be tested for increased performance but ignoring these bounds may result in
   * wrong ordering of display.
   * <p>
   * 
   * @param zIndex
   *            the z index of this trace - the lower the value the more in front the trace will
   *            appear amongst other traces in the same chart.
   * @see #getZIndex()
   */
   ...
   * <li>and at least one of these instances contains at least one {@link IErrorBarPainter}
   * instance.</li>
   * </ul>
   * <p>
   * 
