...
public abstract class AAxis<T extends IAxisScalePolicy> implements IAxis<T>, PropertyChangeListener {
   ...
   * @since 3.0.0
   */
  private abstract static class APropertyChangeReactorSynced implements AAxis.IPropertyChangeReactor {
    ...
    public final void propertyChange(final PropertyChangeEvent changeEvent, final AAxis< ? > receiver) {
      ...
      synchronized (chart) {
        if (Chart2D.DEBUG_THREADING) {
          System.out.println("AAxis.propertyChange (" + Thread.currentThread().getName() + "), 1 lock");
        }
     ...
     */
    protected abstract boolean propertyChangeSynced(final PropertyChangeEvent changeEvent, final AAxis< ? > receiver);
  ...
  public class XDataAccessor extends AAxis<T>.AChart2DDataAccessor {
    ...
    public int getWidth(final Graphics2D g2d) {
       ...
       * For that case the maximum label width is taken into account when
       * finding the right x bound for the chart.
       */
  ...
  protected class YDataAccessor extends AAxis<T>.AChart2DDataAccessor {
    ...
    public final int getHeight(final Graphics2D g2d) {
       ...
       * 
       * In case there are no top x axes a label for an y-axis could be
       * truncated when defining the upper Y-bound for the chart.
       */
      int result = 0;
      if (AAxis.this.isPaintScale()) {
        final FontMetrics fontdim = g2d.getFontMetrics();
   ...
   * To be more precise: It is crucial to use an implementation that will use
   * equals whenever operations like contains are invoked instead of searching
   * by a computed key. In the latter case you could add traces here (at that
   * point in time a key is computed from the trace state) then modify the
   * traces (e.g. adding points) and later when trying to remove the trace the
   * given traces's key would be computed but no key for it found.
   */
  ...
  public AAxis() {
    /*
     * I consider this necessary cast to T as a bug (compare with declaration if
     * T in class header):
     */
    this(new LabelFormatterAutoUnits(new LabelFormatterSimple()), (T) new AxisScalePolicyAutomaticBestFit());
  }
  ...
  public boolean addTrace(final ITrace2D trace) {
    ...
    boolean result = false;
    if (Chart2D.DEBUG_THREADING) {
      System.out.println(Thread.currentThread().getName() + "Aaxis" + this.getDimensionString() + ".addTrace(), 0 locks");
    }
    synchronized (this.getAccessor().getChart()) {
      if (Chart2D.DEBUG_THREADING) {
        System.out.println(Thread.currentThread().getName() + ", AAxis" + this.getDimensionString() + ".addTrace(), 1 lock");
      }
      synchronized (trace) {
        if (Chart2D.DEBUG_THREADING) {
   ...
   * {@link AAxis.AChart2DDataAccessor}</code> implementation.
   * <p>
   * 
   * @param chart
   *          the chart to access.
  ...
  public int hashCode() {
    ...
    result = prime * result + ((this.m_rangePreviousScaling == null) ? 0 : this.m_rangePreviousScaling.hashCode());
  ...
  protected final void scalePoint(final ITracePoint2D point) {
    ...
    if (Chart2D.DEBUG_SCALING) {
      ...
      if ((point.getScaledX() > 1.0) || (point.getScaledX() < 0.0) || (point.getScaledY() > 1.0) || (point.getScaledY() < 0.0)) {
        System.out.println("Scaled Point " + point + " to [" + point.getScaledX() + "," + point.getScaledY() + "]");
      }
    }
  }
   ...
   * <p>
   * <b>This is intended for <code>Chart2D</code> only!</b>. Please do not use
   * this from anywhere in your code. It allows to
   * <p>
   * 
   * @param chart
  ...
  /**
   * Ensures that no deadlock / NPE due to a missing internal chart reference
   * may occur.
   * <p>
   * 
  ...
  public void setRangePolicy(final IRangePolicy rangePolicy) {
    ...
    this.m_rangePolicy.addPropertyChangeListener(IRangePolicy.PROPERTY_RANGE_MAX, this.m_accessor.m_chart);
    this.m_rangePolicy.addPropertyChangeListener(IRangePolicy.PROPERTY_RANGE_MIN, this.m_accessor.m_chart);
  ...
  public void setVisible(final boolean visible) {
    ...
    this.m_propertyChangeSupport.firePropertyChange(IAxis.PROPERTY_VISIBLE, Boolean.valueOf(oldValue), Boolean.valueOf(visible));
  }
