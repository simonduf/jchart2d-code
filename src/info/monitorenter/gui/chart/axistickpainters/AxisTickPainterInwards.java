 ...
 * <p>
 * 
 * @author Verhaag, G.C.H.M.
 * 
 */
public class AxisTickPainterInwards implements IAxisTickPainter {
  ...
  public int getMajorTickLength() {
    ...
    return AxisTickPainterInwards.MAJOR_TICK_LENGTH;
  }
  ...
  public int getMinorTickLength() {
    return AxisTickPainterInwards.MINOR_TICK_LENGTH;
  }
  ...
  public void paintXTick(final int x, final int y, final boolean isMajorTick,
    ...
    if (isMajorTick) {
      if (isBottomSide) {
        g.drawLine(x, y, x, y - AxisTickPainterInwards.MAJOR_TICK_LENGTH);
      } else {
        g.drawLine(x, y, x, y + AxisTickPainterInwards.MAJOR_TICK_LENGTH);
      }
    } else {
      if (isBottomSide) {
        g.drawLine(x, y, x, y - AxisTickPainterInwards.MINOR_TICK_LENGTH);
      } else {
        g.drawLine(x, y, x, y + AxisTickPainterInwards.MINOR_TICK_LENGTH);
      }
    }
  ...
  public void paintYTick(final int x, final int y, final boolean isMajorTick,
    ...
    if (isMajorTick) {
      if (isLeftSide) {
        g.drawLine(x, y, x + AxisTickPainterInwards.MAJOR_TICK_LENGTH, y);
      } else {
        g.drawLine(x, y, x - AxisTickPainterInwards.MAJOR_TICK_LENGTH, y);
      }
    } else {
      if (isLeftSide) {
        g.drawLine(x, y, x + AxisTickPainterInwards.MINOR_TICK_LENGTH, y);
      } else {
        g.drawLine(x, y, x - AxisTickPainterInwards.MINOR_TICK_LENGTH, y);
      }
    }
