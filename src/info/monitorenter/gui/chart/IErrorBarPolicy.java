 ...
 * <p>
 * It acts as a facade for an <code>{@link info.monitorenter.gui.chart.ITracePainter}</code> with
 * configureable <code>{@link info.monitorenter.gui.chart.IErrorBarPainter}</code> instances that
 * will be provided with configured <code>{@link info.monitorenter.gui.chart.IErrorBarPixel}</code>
 * instances.
 * <p>
 * <h3>Property Change events</h3>
 * The following table describes the contract of this interface of
 * <code>{@link java.beans.PropertyChangeEvent}</code> instances that are thrown by methods.
 * <table border="0">
 * <tr>
 ...
 * <td><code>{@link info.monitorenter.gui.chart.IErrorBarPainter}</code>, the added painter.</td>
 * </tr>
 * <tr>
 * <th>{@link #removeErrorBarPainter(IErrorBarPainter)}</th>
 * <td><code>{@link #PROPERTY_ERRORBARPAINTER}</code></td>
 * <td><code>{@link IErrorBarPolicy}</code> that changed</td>
 * <td><code>{@link info.monitorenter.gui.chart.IErrorBarPainter}</code>, the removed painter.</td>
 * <td><code>null</code>, which marks that a painter was removed.</td>
 * </tr>
 * <tr>
 * <th>all mutator methods of {@link info.monitorenter.gui.chart.IErrorBarPainter}</th>
 * <td><code>{@link #PROPERTY_ERRORBARPAINTER}</code></td>
 * <td><code>{@link IErrorBarPolicy}</code> that changed</td>
 * <td><code>{@link info.monitorenter.gui.chart.IErrorBarPainter}</code>, the changed painter.</td>
 * <td><code>{@link info.monitorenter.gui.chart.IErrorBarPainter}</code>, the changed painter
 * (same as old value).</td>
 * </tr>
 * <tr> </table>
 * <p>
 * 
...
public interface IErrorBarPolicy extends ITracePainter {
   ...
   * internal configuration should be of no interest for clients of this interface. What counts is
   * that rendering of the error bars will have changed whenever this event is fired. Subclasses
   ...
   * {@link PropertyChangeListener#propertyChange(java.beans.PropertyChangeEvent)} of listeners
   * should be null always.
   * <p>
  ...
  /**
   * Calculates the errors of the given errorbar according to the point to render and the
   ...
   * @return true if the given error bar painter was removed, comparison by the means of the equals
   *         operation.
   */
  public boolean removeErrorBarPainter(IErrorBarPainter painter);

  /**
   * Allows an implementation to return a <code>JComponent</code> that takes care of custom
   * configuration properties for the UI support of error bar policies.
   * <p>
   * Returns a <code>JComponent</code> that - stand alone - takes care of configuring custom
   * properties or null if nothing is required. This will be integrated in the error bar wizard UI
   * of jchart2d.
   * <p>
   * 
   * @return a <code>JComponent</code> that - stand alone - takes care of configuring custom
   *         properties or null if nothing is required.
   */
  public JComponent getCustomConfigurator();

  /**
   * Returns the relative x error (value domain) that is added to / subtracted from the values to
   * display.
   * <p>
   * 
   * @param xValue
   *            the absolute x value (not pixel) to get the error for.
   * @return the relative x error in value domain that is added to / subtracted from the values to
   *         display.
   */
  public double getXError(final double xValue);

  /**
   * Returns the relative y error (value domain) that is added to / subtracted from the values to
   * display.
   * <p>
   * 
   * @param yValue
   *            the absolute y value (not pixel) to get the error for.
   * @return the relative y error in value domain that is added to / subtracted from the values to
   *         display.
   */
