/*
 *  Trace2DArithmeticMeanX.java of project jchart2d, a trace 
 *  that accumulates the latest n x values added to a single point with the arithmetic 
 *  mean y value and the latest x value. 
 *  Copyright 2005 - 2010 (C) Achim Westermann.
 *
 ...
 * <li>Computing trace: <br/> Add the <code>ITrace2D</code> implementation as a
 * computing trace to an existing trace via
...
public class Trace2DArithmeticMeanX extends ATrace2D {
  ...
  private IRingBuffer<ITracePoint2D> m_pointBuffer;

  /** The internal list of points to render. */
  private List<ITracePoint2D> m_points = new LinkedList<ITracePoint2D>();
   ...
   *          mean.
   */
  public Trace2DArithmeticMeanX(final int arithmenticMeanSpan) {
    super();
  ...
  private ITracePoint2D getArithmeticMean() {
    ...
    for (ITracePoint2D point : this.m_pointBuffer) {
      result = point;
      x += result.getX();
      y = result.getY();
    }
