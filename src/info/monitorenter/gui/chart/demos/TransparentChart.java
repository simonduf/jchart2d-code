 ...
 */
public class TransparentChart extends JFrame {
  /**
  ...
  static class RandomBumper extends Thread {
    ...
    /** The probability of an increase versus a decrease of the bumped number. */
     ...
     * 
     * @param updateTime
     *          the interval for changing values.
     */
    public RandomBumper(final double plusminus, final int factor, final int updateTime) {
    ...
    public void run() {
      ...
      while (true) {
        ...
        try {
          Thread.sleep(this.m_updateTime);
  ...
  public static void main(final String[] args) {
    ...
    ITrace2D data = new Trace2DLtd(800);
    data.setStroke(new BasicStroke(3));
    data.setColor(new Color(255, 0, 0, 255));
    data.setName("random");
    data.setPhysicalUnits("hertz", "ms");

    ITracePainter< ? > painter = new TracePainterPolyline();
    data.setTracePainter(painter);
    chart.addTrace(data);

    TransparentChart wnd = new TransparentChart(chart, TransparentChart.class.getName());
    ...
    wnd.setVisible(true);
    final int speed = 500;
    new ObjRecorder2Trace2DAdapter(data, new RandomBumper(0.5, 1000, speed), "m_number", speed);
  }
   ...
   */
  public TransparentChart(final Chart2D chart, final String label) {
    ...
    this.addWindowListener(new WindowAdapter() {
      ...
      public void windowClosing(final WindowEvent e) {
        ...
        TransparentChart.this.setVisible(false);
        TransparentChart.this.dispose();
        System.exit(0);
