 ...
 * <p>
 * As soon as the maximum number of entries is reached, the next entry is added to the end and the
 * first entry is removed from it. In this case, all elements are stored in a Object[]. But don't
 * worry there will not be a single call to <code>System.arraycopy</code> caused by invocation of
 * the <code>add(Object element)</code>- method. Internal indexes into the array for the head and
 * tail allow to reuse the same memory again and again. <br>
 * No element is lost: If <code>setBufferSize(int asize)</code> decreases the size of the buffer
 * and it will get smaller than the actual amount of elements stored, they will get cached until
 * removed.
 * <p>
...
public class RingBufferArray
  ...
  /**
   * Base class for ringbuffer iterators with pending removals (those who do not drop instances if
   * size is exceeded).
   * <p>
   * 
   * @author <a href="mailto:Achim.Westermann@gmx.de">Achim Westermann</a>
  ...
  abstract class ARingBufferIterator
    ...
    /**
     * The position of the next instance of the pending removed elements to return.
     */
  ...
  /**
   * Elements that stores elements that have to be removed due to an invocation to
   * {@link #setBufferSize(int)} with a smaller argument than the amount of elements stored.
   */
